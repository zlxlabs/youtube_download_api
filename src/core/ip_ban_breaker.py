...
from src.core.ip_ban_models import IPBanLevel, IPBanState
...
class IPBanCircuitBreaker:
    ...
    def get_estimated_recovery_time(self) -> Optional[datetime]:
        ...
        # 计算最早可尝试的时间
        earliest = self.banned_at + timedelta(seconds=self.MIN_WAIT_BEFORE_RETRY)

        # 如果有上次尝试，还要考虑重试间隔
        if self.last_attempt_at:
            retry_after = self.last_attempt_at + timedelta(
                seconds=self.MAX_RETRY_INTERVAL
            )
    ...
    async def upgrade_to_full_ban(self) -> None:
        ...
        当音频熔断期间字幕也失败时调用。
        """
        logger.error("⬆️ Upgrading from audio ban to full ban")
        await self.trigger_full_ban(reason="Transcript also blocked")
